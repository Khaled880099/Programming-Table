...
- `dashboard.html`: Browser dashboard for JSONL events.
...
The blacklist is an application-level block/report only; LogWatch never changes firewall rules. Open `dashboard.html` in a browser, choose the JSONL file produced by a non-following run, and press `LOAD JSONL` to view the event dashboard.
